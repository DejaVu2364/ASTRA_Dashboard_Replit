...
import plotly.graph_objects as go # We need this for the gauge chart
import os
...
# (This section is unchanged and remains the same as the last correct version)
@st.cache_data
...
# --- Pre-calculate metrics for the new cockpit ---
...
primary_comments = df_primary_summary['comment_count'].sum()
primary_posts = df_primary_summary['post_id'].nunique()
...
comp_comments = df_comparison_summary['comment_count'].sum() if not df_comparison_summary.empty else 0

sentiment_map = {-1: 'Negative', 0: 'Neutral', 1: 'Positive'}
sentiment_dist = df_filtered_comments['sentiment_score'].map(sentiment_map).value_counts(normalize=True) * 100
...
# --- !!! THIS IS THE NEW, INTERESTING COCKPIT !!! ---
with overview_tab:
    st.header("Executive Performance Snapshot")
    
    col1, col2 = st.columns([2, 1.5]) # Create a 2-column layout
    ...
    with col1:
        ...
        gauge_fig = go.Figure(go.Indicator(
            ...
            value = primary_sentiment,
            title = {'text': "Avg. Score", 'font': {'size': 20}},
            delta = {'reference': comp_sentiment, 'increasing': {'color': "green"}, 'decreasing': {'color': "red"}},
            gauge = {
                ...
                'bordercolor': "gray",
                'steps': [
                    {'range': [-1, -0.2], 'color': '#FF7C7C'}, # Red
                    {'range': [-0.2, 0.2], 'color': '#FFF87C'}, # Yellow
                    {'range': [0.2, 1], 'color': '#8CFF7C'}], # Green
                'threshold': {
    ...
    with col2:
        st.subheader("Comment Sentiment Mix")
        ...
        donut_fig = px.pie(
            ...
            color_discrete_map={'Positive':'green', 'Neutral':'royalblue', 'Negative':'red'}
        )
        donut_fig.update_traces(textinfo='percent+label', pull=[0.05, 0, 0])
        donut_fig.update_layout(showlegend=False, height=300, margin=dict(l=10, r=10, t=10, b=10))
        st.plotly_chart(donut_fig, use_container_width=True)

    st.markdown("---")
    
    st.subheader("Core Metrics & Period Highlights")
    kpi1, kpi2, kpi3 = st.columns(3)
    kpi1.metric("Average Engagement Rate", f"{primary_engagement:.3%}", f"{primary_engagement - comp_engagement:.3%}" if comparison_option != "None" else None)
    kpi2.metric("Total Comments", f"{primary_comments:,}", f"{primary_comments - comp_comments:,.0f}" if comparison_option != "None" else None)
    kpi3.metric("Total Posts", f"{primary_posts:,}")

    hl1, hl2 = st.columns(2)
    with hl1:
        with st.container(border=True):
            st.markdown("##### 🌟 Top Performing Post")
            top_post = df_primary_summary.loc[df_primary_summary['weighted_engagement_rate'].idxmax()]
            st.markdown(f"**Engagement:** `{top_post['weighted_engagement_rate']:.2%}` | **Sentiment:** `{top_post['avg_sentiment_score']:.2f}`")
            with st.expander("View Caption"):
                st.info(top_post['post_caption'])

    with hl2:
        with st.container(border=True):
            st.markdown("##### ⚡️ Most Controversial Post")
            controversial_post = df_primary_summary.loc[df_primary_summary['sentiment_variance'].idxmax()]
            st.markdown(f"**Variance:** `{controversial_post['sentiment_variance']:.2f}` (High means debate)")
            with st.expander("View Caption"):
                st.error(controversial_post['post_caption'])
# --- END OF NEW COCKPIT ---
...
with analysis_tab:
    ...
    # ... (rest of the tab code is unchanged) ...
    with tab1:
    ...
    with tab2:
        ...
        if 'content_type' not in df_primary_summary.columns or df_primary_summary['content_type'].nunique() <= 1:
            st.info("Enable this chart by adding a 'content_type' column to your data.")
        else:
    ...
    with tab3:
        ...
        if df_comparison_summary.empty or comparison_option == "None":
            st.info("Select a comparison period from the sidebar to view this chart.")
        else:
